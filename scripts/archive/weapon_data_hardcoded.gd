...
static var _use_json: bool = true  # true: JSONを使用, false: ハードコードにフォールバック
...
## 全ConcreteWeaponSetを取得（SSoT対応：JSON優先）
static func get_all_concrete_weapons() -> Dictionary:
	if _use_json:
		_ensure_json_loaded()
		if not _json_weapons.is_empty():
			return _json_weapons
		# JSONが空の場合はハードコードにフォールバック
		push_warning("JSON weapons empty, falling back to hardcoded data")

	return _get_hardcoded_weapons()


## ハードコードされた武器データを取得（フォールバック用）
static func _get_hardcoded_weapons() -> Dictionary:
	return {
		# 汎用
